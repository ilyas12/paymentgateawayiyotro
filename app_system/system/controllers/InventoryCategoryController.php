<?php
class InventoryCategoryController extends AR_Controller{
	...
	public function __construct(){
		...
		$this->controller 	= 'InventoryCategoryController';
		
		$this->view_prefix 	= 'inventoryCategory';
			$this->view_form 	= $this->view_prefix.'Form';
			$this->view_list 	= $this->view_prefix.'List';

		$this->route_prefix = 'inventory_category';
	...
	public function inventoryCategoryList(){
		...
		/*[DB] - GET DB DATA*/
		$data['db_data'] = $this->inventory_category_model->all($searchString,$data_status);
		...
		$data['total'] = $this->inventory_category_model->count($totalSearchString,$data_status);		
	...
	public function addInventoryCategory(){
		...
		if(isset($this->input_data['btnSubmit'])){			
			...
			if($flag != 1){
				...
				/*[DB] - ADD NEW DATA*/
				$newID = $this->inventory_category_model->add($input);
	...
	public function editInventoryCategory($id){
		...
		/*[DB] - GET DB DATA*/
		$data['db_data'] = $this->inventory_category_model->find($id);
		...
		if(isset($this->input_data['btnSubmit'])){
			...
			if($flag != 1){
				...
				/*[DB] - UPDATE DB DATA*/
				$this->inventory_category_model->update($id,$input);
	...
	public function softDelete($id){
		$db_data = $this->inventory_category_model->find($id);		
	...
	public function restore($id){
		$db_data = $this->inventory_category_model->find($id);
		$this->inventory_category_model->restore($id);
